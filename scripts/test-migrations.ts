...
try {
	...
	if (baseEntries.length > 0) {
		...
		// Each supplement_ingredient is backed by a food with kind='supplement';
		// the seed creates 2 such backing foods (shared across 3 ingredients).
		const [{ count: backingFoodCount }] =
			await client`SELECT count(*)::int AS count FROM foods WHERE user_id = ${U1} AND kind = 'supplement'`;
		if (backingFoodCount !== 2) {
			console.error(
				`FAIL: expected 2 supplement backing foods after migration, found ${backingFoodCount}`
			);
			process.exit(1);
		}
		...
		const [{ count: ingredientCount }] =
			await client`SELECT count(*)::int AS count FROM supplement_ingredients si JOIN supplements s ON s.id = si.supplement_id WHERE s.user_id = ${U1}`;
		if (ingredientCount !== 3) {
			console.error(
				`FAIL: expected 3 seeded supplement_ingredients after migration, found ${ingredientCount}`
			);
			process.exit(1);
		}
		console.log(`supplement_ingredients: ${ingredientCount} rows`);

		const [{ count: prefsCount }] =
			await client`SELECT count(*)::int AS count FROM user_preferences WHERE user_id = ${U1}`;
		if (prefsCount !== 1) {
			console.error(
				`FAIL: expected 1 seeded user_preferences after migration, found ${prefsCount}`
			);
			process.exit(1);
		}
		console.log(`user_preferences: ${prefsCount} rows`);
		...
		// they would be handed a brand new empty account on their next sign-in. The
		// check only applies while 0043 itself is under test — once it is part of the
		// base set it runs before seeding and the seeded users are not backfilled.
		if (newTags.some((tag) => tag.startsWith('0043_'))) {
			const backfilled =
