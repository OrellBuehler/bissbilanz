<script lang="ts">
  ...
  const latestTrend = $derived(
    [...chartData].reverse().find((point) => point.moving_avg != null)
      ?.moving_avg ?? null,
  );
...
<div class="mx-auto max-w-4xl space-y-6 pb-8">
    ...
    <Card.Content class="relative p-4 sm:p-6">
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-2 sm:min-w-90 sm:gap-3">
          <div
            class="rounded-xl border border-border/60 bg-background/85 p-3 backdrop-blur"
          >
            <div
  ...
  {:else}
    ...
    <Card.Root class="overflow-hidden">
      <Card.Header
        class="flex flex-row items-center justify-between gap-2 pb-3"
      >
