...
<div class="mx-auto max-w-4xl space-y-6 pb-8">
	<Card.Root class="overflow-hidden border-border/60 bg-gradient-to-br from-blue-50/80 via-background to-emerald-50/60 dark:from-blue-950/20 dark:via-background dark:to-emerald-950/10">
		<Card.Content class="relative p-4 sm:p-6">
			...
			<div class="absolute -bottom-16 left-8 h-32 w-32 rounded-full bg-emerald-500/10 blur-2xl"></div>
			<div class="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
				<div>
					<div class="mb-2 inline-flex items-center gap-2 rounded-full border border-border/60 bg-background/80 px-2.5 py-1 text-xs font-medium text-muted-foreground backdrop-blur">
						<Sparkles class="size-3.5 text-blue-600 dark:text-blue-400" />
						<span>{m.weight_page_title()}</span>
					</div>
					<h1 class="text-2xl font-bold tracking-tight sm:text-3xl">{m.weight_page_title()}</h1>
				</div>
				<div class="grid grid-cols-2 gap-2 sm:min-w-[360px] sm:gap-3">
					<div class="rounded-xl border border-border/60 bg-background/85 p-3 backdrop-blur">
						<div class="text-muted-foreground mb-1 text-[11px] font-semibold uppercase tracking-wider">
							{m.weight_actual()}
						</div>
						<div class="text-sm font-semibold tabular-nums sm:text-base">{formatKg(latestEntry?.weightKg ?? null)}</div>
					</div>
					<div class="rounded-xl border border-border/60 bg-background/85 p-3 backdrop-blur">
						<div class="text-muted-foreground mb-1 text-[11px] font-semibold uppercase tracking-wider">
							{m.weight_trend()}
						</div>
						<div class="text-sm font-semibold tabular-nums sm:text-base">{formatKg(latestTrend)}</div>
					</div>
					<div class="rounded-xl border border-border/60 bg-background/85 p-3 backdrop-blur">
						<div class="text-muted-foreground mb-1 text-[11px] font-semibold uppercase tracking-wider">
							{m.weight_history()}
						</div>
						<div class="text-sm font-semibold tabular-nums sm:text-base">{entries.length}</div>
					</div>
					<div class="rounded-xl border border-border/60 bg-background/85 p-3 backdrop-blur">
						<div class="text-muted-foreground mb-1 text-[11px] font-semibold uppercase tracking-wider">
							{m.weight_chart()}
						</div>
						<div class="text-sm font-semibold tabular-nums sm:text-base">{chartData.length}</div>
					</div>
				</div>
			</div>
		</Card.Content>
	...
	{:else}
		...
		<Card.Root class="overflow-hidden">
			<Card.Header class="flex flex-row items-center justify-between gap-2 pb-3">
				...
				<div class="inline-flex items-center gap-1 rounded-full border border-border/60 bg-background/80 px-2 py-1 text-[11px] font-medium tabular-nums text-muted-foreground">
