...
export const INSIGHT_CARDS: Record<InsightCardId, InsightCardDefinition> = {
	...
	'eating-window': def({
		...
		sources: ['mealTiming30', 'fasts30'],
		component: EatingWindowCard,
		props: (b, loading) => ({ mealTimingData: b.mealTiming30, fasts: b.fasts30, loading })
	}),
	'meal-spacing': def({
		id: 'meal-spacing',
		group: 'nutrition-correlations',
	...
	'plateau-detection': def({
		...
		sources: ['weightFood90', 'weightTarget'],
		component: PlateauDetectionCard,
		props: (b, loading) => ({
			weightFoodData: b.weightFood90,
			targetWeightKg: b.weightTarget.targetWeightKg,
			targetDate: b.weightTarget.targetDate,
			loading
		})
	}),
	'weight-forecast': def({
