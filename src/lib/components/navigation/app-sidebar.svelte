					...
					{#each navItems as item (item.href)}
						<Sidebar.MenuItem>
							<Sidebar.MenuButton
								...
								class="h-12 rounded-xl px-3 text-base transition-colors md:h-8 md:rounded-md md:px-2 md:text-sm data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-foreground hover:bg-sidebar-accent"
							>
								{#snippet child({ props })}
									<a href={item.href} {...withMobileCloseClick(props)}>
										<span
											class="flex size-8 shrink-0 items-center justify-center rounded-lg bg-sidebar-accent text-sidebar-primary md:size-auto md:rounded-none md:bg-transparent md:text-inherit"
										>
