...
const reminder = (overrides: Partial<GeneralReminder> = {}): GeneralReminder => ({
	id: 'rem-1',
	kind: 'weight',
	mealType: null,
	time: '08:00',
...
const dueGeneral = (
	...
	loggedReminderIds: string[] = []
) => dueGeneralReminders({ now, timeZone, reminders, loggedReminderIds });

describe('dueGeneralReminders', () => {
	test('fires when the local wall clock matches the reminder time', () => {
		const result = dueGeneral(new Date('2026-03-10T07:00:00Z'), 'Europe/Zurich', [reminder()]);
		expect(result.map((r) => r.id)).toEqual(['rem-1']);
	});

	test('does not fire a minute early or late', () => {
		expect(
			dueGeneral(new Date('2026-03-10T06:59:00Z'), 'Europe/Zurich', [reminder()])
		).toHaveLength(0);
		expect(
			dueGeneral(new Date('2026-03-10T07:01:00Z'), 'Europe/Zurich', [reminder()])
		).toHaveLength(0);
	});

	test('is skipped when disabled', () => {
		expect(
			dueGeneral(new Date('2026-03-10T07:00:00Z'), 'Europe/Zurich', [reminder({ enabled: false })])
		).toHaveLength(0);
	});

	test('respects the weekday list', () => {
		// 2026-03-10 is a Tuesday (day 2) in Zurich.
		const tuesdayOnly = reminder({ weekdays: [2] });
		const wednesdayOnly = reminder({ id: 'rem-2', weekdays: [3] });
		const result = dueGeneral(new Date('2026-03-10T07:00:00Z'), 'Europe/Zurich', [
			tuesdayOnly,
			wednesdayOnly
		]);
		expect(result.map((r) => r.id)).toEqual(['rem-1']);
	...
	test('suppresses a reminder already satisfied for the local day', () => {
		expect(
			dueGeneral(new Date('2026-03-10T07:00:00Z'), 'Europe/Zurich', [reminder()], ['rem-1'])
		).toEqual([]);
	});

	test('suppresses a reminder already reminded within the same minute', () => {
		const now = new Date('2026-03-10T07:00:40Z');
		const sameMinute = reminder({ lastRemindedAt: new Date('2026-03-10T07:00:05Z') });
		expect(dueGeneral(now, 'Europe/Zurich', [sameMinute])).toHaveLength(0);
	});

	test('a marker from an earlier minute does not suppress', () => {
		const now = new Date('2026-03-10T07:00:00Z');
		const earlier = reminder({ lastRemindedAt: new Date('2026-03-09T07:00:00Z') });
		expect(dueGeneral(now, 'Europe/Zurich', [earlier])).toHaveLength(1);
	});

	test('a meal reminder carries its mealType through untouched', () => {
		const lunch = reminder({ id: 'rem-3', kind: 'meal', mealType: 'Lunch' });
		const result = dueGeneral(new Date('2026-03-10T07:00:00Z'), 'Europe/Zurich', [lunch]);
	...
	describe('DST', () => {
		...
		test('keeps the wall-clock time after the spring-forward transition', () => {
			expect(
				dueGeneral(new Date('2026-03-29T06:00:00Z'), 'Europe/Zurich', [reminder()])
			).toHaveLength(1);
			expect(
				dueGeneral(new Date('2026-03-29T07:00:00Z'), 'Europe/Zurich', [reminder()])
			).toHaveLength(0);
		});
		...
		test('a repeated local hour after the autumn fall-back only fires once', () => {
			...
			const first = new Date('2026-10-25T00:30:00Z');
			const second = new Date('2026-10-25T01:30:00Z');
			const rem = reminder({ time: '02:30' });

			expect(dueGeneral(first, 'Europe/Zurich', [rem])).toHaveLength(1);
			const reminded = reminder({ time: '02:30', lastRemindedAt: first });
			expect(dueGeneral(second, 'Europe/Zurich', [reminded])).toHaveLength(0);
		});
	});

	test('returns every reminder due in the same minute', () => {
		const result = dueGeneral(new Date('2026-03-10T07:00:00Z'), 'Europe/Zurich', [
			reminder(),
			reminder({ id: 'rem-2', kind: 'sleep' })
		]);
		expect(result.map((r) => r.id)).toEqual(['rem-1', 'rem-2']);
	});
});
