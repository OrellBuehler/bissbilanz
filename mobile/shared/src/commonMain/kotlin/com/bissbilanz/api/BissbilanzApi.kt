...
class BissbilanzApi(
    ...
    private suspend inline fun <reified T> authenticatedPost(path: String, body: Any, block: HttpRequestBuilder.() -> Unit = {}): T {
    ...
    private suspend inline fun <reified T> authenticatedPut(path: String, body: Any): T {
    ...
    // Foods
    suspend fun getFoods(limit: Int = 100, offset: Int = 0): List<Food> =
    ...
    suspend fun updateFood(id: String, food: FoodCreate): Food = authenticatedPut("/api/foods/$id", food)
    suspend fun deleteFood(id: String) = authenticatedDelete("/api/foods/$id")
    suspend fun searchFoods(query: String): List<Food> =
        authenticatedGet("/api/foods") { parameter("search", query) }

    suspend fun getRecentFoods(limit: Int = 20): List<Food> =
        authenticatedGet("/api/foods/recent") { parameter("limit", limit) }
    ...
    suspend fun getFoodByBarcode(barcode: String): Food? =
        try { authenticatedGet("/api/foods") { parameter("barcode", barcode) } } catch (_: Exception) { null }

    // Entries
    suspend fun getEntries(date: String): List<Entry> =
        authenticatedGet("/api/entries") { parameter("date", date) }

    suspend fun createEntry(entry: EntryCreate): Entry = authenticatedPost("/api/entries", entry)
    suspend fun updateEntry(id: String, entry: EntryUpdate): Entry = authenticatedPut("/api/entries/$id", entry)
    ...
    suspend fun updateRecipe(id: String, recipe: RecipeUpdate): Recipe = authenticatedPut("/api/recipes/$id", recipe)
    suspend fun deleteRecipe(id: String) = authenticatedDelete("/api/recipes/$id")

    // Goals
    suspend fun getGoals(): Goals? =
        try { authenticatedGet("/api/goals") } catch (_: Exception) { null }
    ...
    suspend fun getWeightEntries(limit: Int = 30): List<WeightEntry> =
        authenticatedGet("/api/weight") { parameter("limit", limit) }
    suspend fun createWeightEntry(entry: WeightCreate): WeightEntry = authenticatedPost("/api/weight", entry)
    suspend fun updateWeightEntry(id: String, entry: WeightUpdate): WeightEntry = authenticatedPut("/api/weight/$id", entry)
    ...
    suspend fun logSupplement(supplementId: String, date: String? = null): SupplementLog =
        authenticatedPost("/api/supplements/$supplementId/log", mapOf("date" to date))
    ...
    // Stats
    suspend fun getDailyStats(startDate: String, endDate: String): DailyStatsResponse =
    ...
    suspend fun getTopFoods(days: Int = 7, limit: Int = 10): TopFoodsResponse =
    ...
    // Preferences
    suspend fun getPreferences(): Preferences = authenticatedGet("/api/preferences")
