...
struct SnapshotProvider: TimelineProvider {
    ...
    func getSnapshot(in _: Context, completion: @escaping (SnapshotTimelineEntry) -> Void) {
        completion(SnapshotTimelineEntry(date: Date(), snapshot: WidgetSnapshotStore.currentSnapshot(at: Date())))
    }

    func getTimeline(in _: Context, completion: @escaping (Timeline<SnapshotTimelineEntry>) -> Void) {
        let now = Date()
        var entries = [SnapshotTimelineEntry(date: now, snapshot: WidgetSnapshotStore.currentSnapshot(at: now))]
        ...
        if let midnight = Calendar.current.nextDate(
            ...
            entries.append(SnapshotTimelineEntry(date: midnight, snapshot: WidgetSnapshotStore.currentSnapshot(at: midnight)))
        }
